...
part '{{singular.pascalCase()}}_repository.g.dart';
...
class {{singular.pascalCase()}}RepositoryImpl extends PBCollectionRepository<{{singular.pascalCase()}}> {
  ...
  RecordService get collection => pb.collection(PocketBaseCollections.{{singular.camelCase()}}es);
  ...
  TaskResult<void> softDeleteMulti(List<String> ids) {
    return TaskResult.tryCatch(() async {
      ...
      final batchCollection = batch.collection(PocketBaseCollections.{{plural.camelCase()}});
      for (final id in ids) {
        batchCollection.update(id, body: {{{singular.pascalCase()}}Field.isDeleted: true});
      }
